...
function workerMain() {
  ...
  function hasRedundantWideOuterSandwich(moves) { const cleaned = cleanMoves(moves); const wideToOuter = { r: "R", l: "L", u: "U", d: "D", f: "F", b: "B" }; const powerOf = (move) => move.endsWith("2") ? 2 : move.endsWith("'") ? 3 : 1; const inversePower = (a, b) => ((powerOf(a) + powerOf(b)) % 4) === 0; for (let i = 0; i + 2 < cleaned.length; i += 1) { const a = cleaned[i]; const b = cleaned[i + 1]; const c = cleaned[i + 2]; const outer = wideToOuter[a[0]]; if (!outer) continue; if (b[0] === outer && c[0] === a[0] && inversePower(a, c)) return true; } return false; }
  function readabilityPenalty(moves) { let penalty = 0; const cleaned = cleanMoves(moves); for (const move of cleaned) { const face = move[0]; if ("xyz".includes(face)) penalty += 80; if ("MES".includes(face)) penalty += 20; } return penalty; }
  function emitSolution(job, solution) { const normalized = trimRedundantFinalAuf(job, solution); if (hasRedundantWideOuterSandwich(normalized)) return false; if (!solutionMatchesRequiredParts(normalized, job.requiredParts)) return false; const key = algToString(normalized); if (job.foundKeys.has(key)) return false; job.foundKeys.add(key); job.foundCount += 1; self.postMessage({ type: "solution", solution: normalized }); if (job.foundCount >= job.maxResults * 4 && readabilityPenalty(normalized) > 0) return true; if (job.foundCount >= job.maxResults) job.stopByLimit = true; return true; }
